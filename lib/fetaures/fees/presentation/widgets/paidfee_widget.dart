...
class _PaidFeeState extends State<PaidFee> {
  ...
  Widget build(BuildContext context) {
    if (widget.feePaidResult.data.isNotEmpty) {
      return ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final String paymentMode = fee.paymentMode.toString().trim();

          final bool hasPaymentMode =
              paymentMode.isNotEmpty && paymentMode.toLowerCase() != 'null';
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (isExpanded) ...[
                            ...
                            //   ],
                            // ),
                            _expandedFeeTitleRow(),

                            const SizedBox(height: 4),
                            Column(
                              children: fee.details.map<Widget>((dtls) {
                                return _expandedFeeRow(
                                  ledger: dtls.ledgerName.toString(),
                                  month: dtls.feeMonth.toString(),
                                  amount: dtls.paidAmount.toString(),
                                );
                              }).toList(),
                            ),
                          ],
...
class TicketClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    final leftCutout = Path()
      ..addOval(
        Rect.fromCircle(center: Offset(0, cutoutCenterY), radius: cutoutRadius),
      );

    final rightCutout = Path()
      ..addOval(
        Rect.fromCircle(
          center: Offset(size.width, cutoutCenterY),
          radius: cutoutRadius,
        ),
      );

    final pathAfterLeftCut = Path.combine(
      ui.PathOperation.difference,
      basePath,
      leftCutout,
    );
    final finalTicketPath = Path.combine(
      ui.PathOperation.difference,
      pathAfterLeftCut,
      rightCutout,
    );

    return finalTicketPath;
  }
...
class _DashedDividerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    while (startX < size.width) {
      ...
      canvas.drawLine(
        Offset(startX, 0),
        Offset(endX > size.width ? size.width : endX, 0),
        paint,
      );
